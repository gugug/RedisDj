...
from RedisDj.settings import BASE_DIR
import sys
...
def combine_txt(file_name, slave_num):
    ...
    print stats_txt
    file_num = len(get_file_list(file_name, []))
    print "文本数目",file_num
    if file_num == slave_num:
        for t in range(1,file_num+1):
            txt = open(stats_txt + '/stats' + str(t)+'.txt', 'r')
            words = txt.readlines()
            for i in range(len(words)):
                words[i] = words[i].replace('\n', '')
                wn = words[i].split(',')
    ...
    dump_map_json(file_name, map_xls_file)
    dump_age_json(file_name, age_json_list)
...
# combine_txt('/home/gu/PycharmProjects/RedisDj/documents/topic/中国南海问题', )
