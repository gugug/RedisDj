...
import redis
import time
from Crawler.combine_txt import combine_txt
from Crawler.scp_statstxt import scp_to_master
from RedisDj.settings import *
from crawl_headhunter import Sina
from stats_all import StatsAll
from Crawler.RedisQueue import RedisQueue
...
def get_redis_key():
    ...
    :return:
    """
    db = redis.Redis(host='127.0.0.1', port=6379, db=0)  # 192.168.235.51
    if db.dbsize():
        kes = db.keys("*")
        for i in kes:
...
def sleep_time():
    ...
    zero_clock = datetime.datetime(today.year,today.month,today.day,0,0,0)
    tomorrow = zero_clock + datetime.timedelta(days=1)
...
def main_down_stats(SLAVENUM, YOURID):
    ...
    while True:
        if get_redis_key():
            for topic, abs_path in downresult_dir.items():
                print topic
                redis_event = RedisQueue(topic)

                mul_stats(SLAVENUM, YOURID, redis_event, abs_path)
                scp_to_master(abs_path, YOURID) # slave上传文本
            ...
            print "全部任务完成，正在睡眠...直到第二天凌晨,睡眠秒数为", wait_time
